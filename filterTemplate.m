function [xhat, meas] = filterTemplate(L, Ra, Rm, Rw)
...
% Note that it is not necessary to provide inputs (calAcc, calGyr, calMag).
  ...
  alpha = 0.01;      %%%%%%%%%%%%%%%%%%%%% change it here
  g0 = [0;0;9.81];
  trh_acc = 0.2;   %%%%%%%%%%%%%%%%%%%%%% change it here
  trh_mag = 15;   %%%%%%%%%%%%%%%%%%%%%% change it here
  ...
  try
    ...
    while server.status()  % Repeat while data is available
      ...
      if ~any(isnan(mag))  % Mag measurements are available.
        ...
        if abs(norm(mag)-L)<trh_mag   
           magOut = 0;               
           % update
           m0 = [0 norm([mag(1) mag(2)]) mag(3)];
     ...
     %% plot 
      euler1 = q2euler(x);       %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% how to plot it
      euler2 = q2euler(orientation);
